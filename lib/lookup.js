...
export default class OdataLookup extends OdataValue {
   ...
   */
  static replaceInData (data, { key, type, expand, expandText, options }, instanceOptions) {
    if (expand) {
      data[key] = new this(
        data[key],
      ...
      data[key] = new this(value, text, { ...instanceOptions, type })
    } else {
      data[key] = new this(data[key], '', { ...instanceOptions, type })
    }
    return data
  }
